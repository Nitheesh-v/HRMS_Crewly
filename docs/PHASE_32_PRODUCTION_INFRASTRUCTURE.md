...
Status: **32.5 implemented** (awaiting localhost acceptance). Evidence
first: the campaign is driven by the queries the platform actually runs
(file:line citations), cross-checked against the schema declarations
MongoDB itself builds — not by speculation.

## METHOD

The sandbox has no `mongod`; Mongo-backed runtime tests execute on
developer localhost (standing law). The 32.5 evidence tier is therefore
**static and hermetic**: Mongoose schema declarations ARE the ground
truth of what MongoDB builds. `npm run index:check` loads all **124
model modules** with zero connections and reports:

- inventory: **615 declared indexes across 124/124 collections**
  (unique: 105, TTL: 11, partial: 25) — zero index-free collections;
- the **hot-query catalog**: 16 pinned query paths (each with file:line
  evidence) evaluated under a documented conservative rule — equality
  keys must be an in-order PREFIX of some index; `findOne` is also
  point-served when ANY equality key is UNIQUE-led (candidate set ≤ 1).

## VERDICT TABLE (the 16 pinned hot paths)

| Query path | Collection | Verdict |
| --- | --- | --- |
| `protect` session validation (EVERY request) | SecuritySession | COVERED (point: unique `sessionId`) |
| refresh-token rotation | SecuritySession | COVERED (point: `_id`) |
| logout-all / revoke by user | SecuritySession | COVERED (point: unique `sessionId`) |
| Super Admin session validation (EVERY platform request) | AdminSession | COVERED (point: unique `sessionId`) |
| kiosk station auth | AttendanceKiosk | COVERED (point: `_id`) |
| attendance day events (+sort) | AttendanceEvent | FILTER+SORT (`{companyId,user,date,seq}` unique) |
| payroll run lookup | PayrollRun | COVERED (unique `{companyId,month}`) |
| payslip tenant lists | Payslip | COVERED (`{companyId,month,status}`) |
| notification inbox | Notification | COVERED (`{user,readAt}` / `{user,createdAt:-1}`) |
| subscription lifecycle sweep (10 s) | Subscription | **DOCUMENTED** — `$nin` is not index-bound; collection is one doc per company (tiny); query byte-frozen |
| public careers list | JobPosting | COVERED (`companyId` index; per-tenant sort bounded) |
| login user lookup | User | COVERED (unique `{email,companyId}`) |
| candidate pipeline board | Candidate | COVERED (`{companyId,job,currentStage,source}`) |
| BGV case dispatcher polling | BackgroundVerificationCase | COVERED (`{companyId,polling.status,polling.nextPollAt}`) |
| BGV checks per case | BackgroundVerificationCheck | COVERED (unique `{companyId,case,code}`) |
| pre-onboarding document requirements | CandidateDocumentRequirement | COVERED (unique `{companyId,preOnboarding,code}`) |

**Outcome: ZERO missing critical indexes — no schema changes were
needed.** Index hygiene from phases 27–31 held up under audit; the
campaign's deliverable is that this stays true.

## GUARDS (the campaign is now a tested property)

`npm run test:index-coverage` (13 hermetic tests) — imports the ONE
catalog from `scripts/index-check.js`, so tool and tests cannot drift:

1. no catalog entry may become a GAP;
2. exact key-sequence pins for crown jewels (session unique indexes,
   attendance day-sequence unique + sparse replay, payslip/payroll,
   login, BGV, notification partial-unique dedupe);
3. **tenant-first law**: 9 high-volume collections must keep a
   `companyId`-led compound index;
4. TTL housekeeping (sessions, tokens, audit log);
5. **source-shape pins**: the cataloged queries must still exist in
   code verbatim (query ↔ index contract rot guard).

## OPERATOR RECIPES (real data — developer localhost only)

```js
// mongosh — confirm a pinned path is index-served with real volumes:
db.securitysessions.explain("executionStats").findOne(
  { sessionId: "<real-id>", user: ObjectId("…"),
    companyId: ObjectId("…"), revokedAt: null,
    expiresAt: { $gt: new Date() } })
// → winningPlan IXSCAN on sessionId_unique; totalDocsExamined ≤ 1

// Which indexes real traffic actually uses (run over a day):
db.attendanceevents.aggregate([{ $indexStats: {} }, { $match: { "accesses.ops": { $gt: 0 } } }])
```

Index builds on large production collections: MongoDB 4.2+ builds are
non-blocking; still schedule first builds in a quiet window (32.15).

## IMPLEMENTED / TESTED / DEFERRED

- IMPLEMENTED + TESTED: `scripts/index-check.js` (`npm run
  index:check`, secret-free, exit 1 on GAP), `test/indexCoverage.test.js`
  13/13, hot-query catalog with evidence citations, verdict table above.
- DEFERRED: real-data `explain`/`$indexStats` baselines (needs real
  volumes — localhost/production operator step); catalog expansion
  beyond the 16 pinned paths (the auditor makes adding entries a
  one-line change); slow-query profiling under load → 32.13/32.14
  campaigns; index-tuning decisions that require production stats.

# 32.4 — Distributed Rate Limiting (Multi-Instance Budgets)

Status: **32.4 implemented** (awaiting localhost acceptance). Phase 32.3
made every limiter key on honest client identity; 32.4 makes those keys
COUNT against one shared budget when the platform runs as API #1/#2/#N.

## WHY

Every `securityRateLimit` bucket was a process-local `Map`. Behind two
API instances an attacker gets 2× the login attempts, 2× the OTP/reset
guesses, 2× the public-form floods. 32.4 moves SECURITY-SENSITIVE
counters into Redis so every instance enforces the SAME budget — while
keeping the pre-32.4 per-process behavior as the guaranteed degraded
mode. Redis is coordination, never truth: a degraded Redis NEVER means a
weaker limit (it means a local bucket instead).

## THE ONE STORE

`Backend/src/utils/rateLimitStore.js` — `createRateLimitStore({ sharedName, windowMs, io? })`

- Key layout: `crewly:<env>:rl:<sharedName>:<identity>` via
  `getQueuePrefix()` — env-isolated, surface-scoped, tenant-scoped
  (identity comes from 32.3's trust-aware `req.ip`, so limiter identity
  is impossible to forge).
- `hit(identity, maximum)` → `{ limited, count, remaining, resetAt,
  tier: 'shared' | 'local' }`. Fixed window: `INCR` + `EXPIRE NX`
  (only the first hit in a window sets the TTL; fallback
  `count === 1 → EXPIRE` when the server lacks `EXPIRE NX`, Redis < 7).
- Degradation ladder, ALL internally bounded (no new env names):
  Redis disabled / unreachable / error / op > 250 ms → **process-local
  circuit breaker opens 30 s** → identical in-process buckets
  (bounded at 10 000 keys, oldest evicted). Limit is NEVER skipped.
- Only exact keys + TTL are ever touched — no `KEYS`/`SCAN`/`FLUSH`
  anywhere in the path (test-pinned).

## MIDDLEWARE CONTRACT (byte-frozen)

`securityRateLimit` gains an OPTIONAL `sharedName`. The 429 body
(`RATE_LIMITED` + message) and `X-RateLimit-Limit / -Remaining / -Reset`
headers are IDENTICAL in local and shared tiers. Without `sharedName`
the limiter behaves exactly as before. No limiter UX changed; the
account-lockout mechanism is untouched (it is DB-based, not Redis).

## LIMITER INVENTORY — shared vs local (decision record)

| Surface | sharedName | Rationale |
| --- | --- | --- |
| Auth login / refresh / forgot / reset (`authRoutes` exported limiters) | `login`, `refresh`, `password-reset`, `password-change` | credential-guessing surfaces |
| Kiosk session / punch / identify | `kiosk-session`, `kiosk-punch`, `kiosk-identify` | public, tenant-scoped |
| Kiosk PIN management | `kiosk-pin-manage` | credential surface |
| BGV verifier login / recovery / work | `bgv-verifier-login`, `bgv-verifier-recovery`, `bgv-verifier-work` | credential + public work surfaces |
| Public BGV consent read/decision | `bgv-consent-read`, `bgv-consent-decision` | public token surfaces |
| Public BGV collection read/write/upload/submit | `bgv-coll-read/write/upload/submit` | public token surfaces |
| Public candidate offer read/decision | `offer-read`, `offer-decision` | public token surfaces |
| Pre-onboarding read/upload | `preonb-read`, `preonb-upload` | public token surfaces |
| Careers list / application submit | `careers-read`, `careers-apply` | public flood surfaces |
| Super Admin login guard | `super-admin-login` | 5 failures → 15-min block, now shared across instances |
| Recruitment resume-reprocess + other authenticated low-stakes limits | STAYS LOCAL | authenticated, low abuse value; Redis hop avoided on hot internal paths |

Super Admin guard: `createSuperAdminLoginGuard({ store? })` factory is
additive; the default export `superAdminLoginGuard` is unchanged for
routes (5 failures / 15-minute block / clear on success contract
byte-frozen, now enforced on the shared counter).

## DEGRADATION STORY (operator summary)

| Redis state | Behavior |
| --- | --- |
| healthy | one shared budget across all instances |
| down / slow / erroring | circuit opens ≤ 30 s → per-process buckets (bounded 10k) — the exact pre-32.4 behavior, never weaker than baseline |
| disabled (`REDIS_ENABLED=false`) | always local buckets — dev/localhost unchanged |

## TESTS & OPERATIONS

- `npm run test:rate-limit` — 11 hermetic tests: shared budget across
  TWO middleware instances, identity/surface/env key isolation, disabled
  → local, error → circuit → local, window expiry, super-admin
  shared/clear, wiring pins (sharedName count per route file, no
  KEYS/SCAN/FLUSH, bounded fallback constants).
- `npm run test:all` — 1616 tests / 1605 pass / 11 fail (the 11 are the
  long-standing no-Mongo baseline files; unchanged since 32.1).
- Zero new packages, zero new env names (250 ms op timeout / 30 s
  circuit / 10k fallback bound are internal constants).
- Localhost proof (PowerShell): fire 6 rapid login POSTs and read
  `X-RateLimit-Remaining` from `$_ .Exception.Response.Headers`; with
  `REDIS_ENABLED=false` buckets are local — behavior identical to
  pre-32.4.

## IMPLEMENTED / TESTED / DEFERRED

- IMPLEMENTED + TESTED: shared store, dual-tier middleware, Super Admin
  factory, all 20 shared wirings, degradation ladder, hermetic suite.
- DEFERRED: sliding-window/precision quota algorithms (fixed window is
  the shipped, documented semantics); per-account (not per-IP) global
  quotas (would need product decisions); live two-instance demo on a
  real Redis (sandbox has no Redis — the store's real Redis path is
  exercised only for disabled/fallback behavior here; `npm run
  test:redis` pins the foundation contract).
