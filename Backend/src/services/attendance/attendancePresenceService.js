...
//
// Phase 31.12 seam (default-off, internal only): `includeAllRows`
// additionally returns the pre-pagination derived rows and
// `opsFacts` adds authoritative late/early minutes per row, so the
// HR operations dashboard can aggregate WITHOUT re-deriving
// presence. Default output is byte-identical to 31.9.
// ─────────────────────────────────────────────────────────────
...
const serializeRow = ({ person, departmentName, derived, locationName, ops = null }) => ({
  user: {
    id: strId(person._id),
    name: person.name || '',
  ...
  // 31.12 ops facts (only when the internal caller opts in):
  // authoritative 31.1/31.6 minutes, never recomputed here.
  ...(ops ? { lateMinutes: ops.lateMinutes, earlyMinutes: ops.earlyMinutes } : null),
...
export const getTeamPresence = async ({
  ...
  const rows = (users || []).map((person) => {
    ...
    return serializeRow({
      ...
      ops: opsFacts
        ? {
          lateMinutes: Number(primary?.lateMinutes) || 0,
          earlyMinutes: Number(primary?.earlyMinutes) || 0,
        }
        : null,
    });
  });
  ...
  return {
    ...
    total,
    totalPages,
    ...(includeAllRows ? { allRows: filtered } : null),
  };
};
