...
const TemplateForm = ({ template, variables, onClose, onSaved }) => {
  ...
  const submit = async (event) => {
    ...
    setError('');
    try {
      const saved = template ? await offerService.updateTemplate(template._id, form) : await offerService.createTemplate(form);
      onSaved(saved);
    } catch (requestError) {
...
const OfferTemplatesPage = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setVariables(result.supportedVariables);
    } catch (requestError) {
  ...
  const deactivate = async (template) => {
    ...
    try {
      await offerService.deactivateTemplate(template._id);
      load();
    } catch (requestError) {
  ...
  const activate = async (template) => {
    try {
      await offerService.updateTemplate(template._id, { isActive: true });
      load();
    } catch (requestError) {
